...
**What nanogent is.** A minimum bridge between an existing agent harness (Claude Code, opencode, codex, cursor-agent, aider, or a raw LLM provider) and common human channels (Telegram, WhatsApp, email, web). Nanogent owns channel ingress, identity and permissions, turn orchestration, proactive turn injection, and the plugin lifecycle that makes those composable. It does **not** own skills, subagents, sandboxed code execution, file editing, code search, browser automation, or prompt engineering — those belong to whichever harness a `tools/<harness>` plugin wraps. See [DR-017](#dr-017-nanogent-is-a-minimum-bridge-not-an-agent-harness).
...
**Coverage claim.** In combination with `ToolPlugin.start()` ([DR-010](#dr-010-proactive-triggers-live-inside-tool-plugins-via-the-lifecycle-hook)) and `ToolPlugin.contributeContext()` ([DR-016](#dr-016-memory-is-not-a-plugin-type-tools-contribute-context-via-a-hook)), the primitives exposed through `ToolCtx` / `ToolStartCtx` / `ToolContextCtx` are sufficient to express every plugin use case the core currently anticipates:

- **Reactive capabilities** — agent-invoked tools via `execute()` (search, file ops, task CRUD, shell-outs to vendor CLIs).
- **Proactive triggers** — schedulers, webhook listeners, file watchers, inbox pollers, log tailers, queue consumers. Own the loop inside `start()`, call `ctx.fireSystemTurn` when there is something to say, return a stop fn.
- **Derived-context injection** — RAG retrievers, summary memory, clock, location, project-status, schedule-awareness. Return system-prompt text from `contributeContext()`.
- **Derived-index maintenance** — tools that hold a view over history (vector indexes, summaries) stay in sync via `onHistoryAppended` / `onHistoryRetracted` / `onClear`, and backfill on boot by walking `ctx.history` inside `start()`.
- **Task-assignment / work-queue tools** — persist tasks under `pluginDir/state/`, ping the user on completion via `ToolCtx.sendMessage`, fire the next pickup via `ctx.fireSystemTurn` (targeting the channel/chatId/contactId recorded when the task was created). This is the same shape as `tools/schedule`, with "task-complete" replacing "next cron tick" as the trigger.

The operational rule that falls out of the claim: **new use cases should be attempted against these seams first; a new plugin type is the last resort and must name a primitive that no existing ctx + hook combination can provide.** The last three times a new plugin type felt necessary (scheduler in v0.7.0, memory in v0.5.0, both reversed), the actual need was a hook on an existing type. Future extension work starts by trying the hook path and only introduces a new plugin type when the hook path demonstrably cannot carry the weight.
...
- [ ] Before proposing a new plugin type, walk the Coverage claim above and name a primitive the existing ctx + hook combination genuinely cannot express. "It would be cleaner as its own type" is not sufficient — that argument was made for scheduler and memory and was wrong both times. Default to extending an existing hook or adding one new hook on an existing plugin type.
...
### DR-017: Nanogent is a minimum bridge, not an agent harness

**Status:** Accepted (retroactive — ratifies the scope the codebase has held since v0.3.x and sharpens it into an explicit rule).

**Principle:** Nanogent is the minimum bridge between an existing agent harness and common human channels (Telegram, WhatsApp, email, web, future transports). It provides the things harnesses typically don't — channel ingress, stable identity, permission resolution, proactive turn injection, multi-user routing — and delegates everything an agent harness *does* provide (skills, subagents, sandboxed tool execution, file editing, code search, browser automation, context engineering, prompt optimisation) to whichever harness a `tools/<harness>` plugin wraps.

**Context:** The agent-harness landscape in 2026 is already mature and still evolving quickly. Claude Code ships skills, subagents, hooks, MCP, sandboxed execution, a large built-in tool library, and years of prompt engineering. OpenAI's Agents SDK, `codex`, `cursor-agent`, `opencode`, `aider`, Gemini CLI, and others occupy similar territory with different trade-offs. Reimplementing any of that inside nanogent would be a multi-year project that ends up strictly worse than the thing it copied and locks nanogent to one mental model. Meanwhile, the thing most of those harnesses *don't* ship — "make this agent proactively reachable by a human on their phone, via their existing messenger, with stable identity and permissions across channels" — is exactly what nanogent provides.

Once you accept that framing, the scope rule writes itself: nanogent owns the bridge, harnesses own the thinking. The bundled `tools/claude` is already a thin shell-out that does not try to mediate Claude Code's tool use, skills, or subagents — it hands the user's text to `claude` and returns the reply. This DR names that shape as the intended one and forbids drift toward a "nanogent skill system" / "nanogent subagents" / "nanogent built-in code search" future.

**What nanogent owns (the bridge surface):**

- **Multi-channel ingress.** Telegram, WhatsApp, email, web, future channels. Normalising every transport into one message shape. See DR-001/DR-002/DR-004/DR-008.
- **Identity and permissions.** `contacts.json`, shared vs per-user history, chat allowlist, guest tools. See DR-005/DR-006.
- **Turn orchestration.** User turn → provider call → tool loop → channel reply. Plus the boundary-aware history windowing that makes this correct across providers (core invariant, not pluggable — see DR-016).
- **Proactive turn injection.** One primitive — `fireSystemTurn` — handed to any tool that wants to speak without being prompted. Scheduler, webhook listener, file watcher, inbox poller, task queue all share this. See DR-010/DR-014.
- **Plugin lifecycle + state ownership.** The five-interface plugin system (tool, channel, provider, history + `start()` / `contributeContext` / history hooks), minimal ctx coupling, plugin-owned state. See DR-013/DR-014/DR-015/DR-016.
- **Append-only history log.** A portable canonical record, independent of any harness's internal memory. See DR-009a/DR-015.

**What nanogent deliberately does NOT own (delegated to the wrapped harness):**

- **Skills / prompt packs.** Claude Code's `.claude/skills/` system, OpenAI Assistants' instructions, cursor-agent's rules, opencode's modes — each harness has its own mental model and storage. Nanogent wraps whichever one the operator chose; it does not ship a competing one.
- **Subagents / delegation / multi-agent orchestration.** Claude Code subagents, LangGraph agents, OpenAI Swarm — all belong to the harness.
- **Sandboxed code execution.** `claude --bypass-permissions`, OpenAI code interpreter, E2B sandboxes — harness concern.
- **File editing / code search / codebase indexing.** `str_replace_editor`, ripgrep, glob, language servers — harness concern.
- **Browser automation.** Puppeteer, Playwright, Computer Use — harness concern.
- **Prompt engineering primitives.** Cache management beyond the base system prompt shape, chain-of-thought prompting, few-shot templating, structured output schemas — harness concern.
- **Context compression / summarisation beyond boundary-aware windowing.** If an operator wants smarter context, they install a `contributeContext` tool plugin (DR-016) or switch to a harness-tool that handles it internally.

The dividing line is crisp: **if the feature is about getting the agent to humans, it belongs in nanogent. If the feature is about how the agent thinks, it belongs in whichever harness a tool plugin wraps.**

**How this shows up in the code (v0.12.0):**

- **`tools/claude` is a thin shell-out.** It doesn't parse Claude Code's config, doesn't route around its tools, doesn't reimplement its skills. It calls the `claude` CLI binary installed by `tools/claude/install.sh` (DR-011) with the user's text and returns the reply. Everything Claude Code does internally — tool use, skills, subagents, hooks, MCP — happens invisibly to nanogent.
- **The built-in provider tool loop is the lowest common denominator, not the preferred path.** `nanogent.ts`'s message-exchange loop exists for providers that expose the raw LLM API without an agent harness on top (Anthropic Messages API, OpenAI Chat Completions, etc.). It implements just enough — message threading, tool-use parsing, tool-result injection — to let those providers work at all. It is explicitly *not* where nanogent should invest in harness-level features; operators who want harness features install a harness-wrapping tool plugin instead.
- **Future harness wrappers drop in as tool plugins with zero core changes.** `tools/opencode/`, `tools/codex/`, `tools/cursor-agent/`, `tools/aider/`, `tools/gemini-cli/`, `tools/goose/` — each one is a `ToolPlugin` that shells out to its binary. `install.sh` (DR-011) handles container-side installation. `resources.json` (DR-011 advisory) declares memory/CPU floors. No new plugin type, no core edits. An operator switches harnesses by swapping the tool directory in their profile.

**Consequences.**

- **Feature-request filter.** Every proposed core feature is evaluated against the same question: "is this bridging infrastructure (channels, identity, proactive triggers, lifecycle, history) or is this harness work (skills, subagents, sandboxing, context engineering, built-in tools)?" Bridging → consider it. Harness → redirect to "which harness tool plugin do you use, and does *it* do X?" Over time this keeps nanogent small and forces the hard feature work onto the harness layer where it already has maintainers and budget.
- **No "nanogent skill system."** If a future contributor proposes `.nanogent/skills/` with markdown files, system-prompt composition rules, or metadata frontmatter, the answer is: that's Claude Code's skill system; wrap Claude Code; do not re-ship it.
- **No "nanogent subagents."** Claude Code subagents, LangGraph, LlamaIndex, CrewAI, OpenAI Swarm — all are harness-level primitives. Multi-agent orchestration inside nanogent would duplicate that.
- **No built-in file-edit / code-search / browser tools.** They exist in every harness already. Shipping them inside nanogent both reinvents the wheel and competes with the harness the operator actually chose.
- **The built-in provider tool loop stays minimal.** No "rich tool planner," no "automatic retry with reflection," no "tool-selection LLM call." If an operator needs that behavior, they should use a harness tool plugin instead of a raw provider. The raw-provider path is the safety net for operators who genuinely want to run against the bare LLM API and accept the trade-offs.
- **DR-014's coupling principle applies doubly here.** Core must not know anything about any specific harness's concept of "capability." A tool plugin is a black box that takes text and returns text (plus side effects inside its own container/process). Anything richer introduces coupling to *that* harness's shape.
- **The `Coverage claim` in DR-014 remains honest because harness work is out of scope.** The claim is "the three-hook surface covers every plugin use case the core currently anticipates." That claim would collapse if "core" also tried to own skills, subagents, sandboxing, etc. Keeping those delegated is what makes the coverage claim defensible.

**Alternatives considered (and rejected):**

- **Ship a nanogent skill system.** Markdown skill files in `.nanogent/skills/`, composed into the system prompt per turn, possibly with per-user / per-chat scoping. Rejected: Claude Code already has a skill system with years of iteration, tooling, and operator mindshare. Shipping a second one forks the operator's attention and creates bidirectional drift (every Claude Code skill improvement makes nanogent's look worse; every nanogent skill improvement creates a porting tax for operators who also use Claude Code directly). The right answer is "write your skill as a Claude Code skill; `tools/claude` picks it up automatically." For non-Claude harnesses, each harness has its own equivalent.
- **Ship nanogent-native subagents / delegation.** A way for the agent to spin up child contexts with scoped tool access. Rejected: this is the single most differentiated feature across harnesses right now (Claude Code subagents, LangGraph StateGraph, OpenAI Swarm handoffs, CrewAI crews all have different mental models). Picking one locks nanogent to that model; building our own means we're now a harness vendor, competing with five teams who have more resources. Delegate it to the harness layer.
- **Ship built-in coding tools (file_edit, grep, glob, bash, browser).** Rejected: every coding-focused harness ships these, usually with sandbox integration and safety policies nanogent would have to reimplement. Users who want to code from Telegram install `tools/claude` (or `tools/opencode`, or `tools/codex`) and get the full set through the wrapped binary. Users who don't need coding — customer support, scheduling, research assistants — shouldn't carry the weight of an unused coding toolkit.
- **Ship a rich tool planner / reflection loop on top of the raw provider path.** "Try tool → observe result → reflect → retry with adjustment." Rejected: this is exactly the work every agent harness is built around. If an operator needs that behavior, the right answer is to install a harness tool plugin, not to turn nanogent into a second-rate harness. The raw-provider path is a fallback for operators who explicitly want minimalism.
- **Ship a context engineering framework** (pluggable compression, rolling summaries, hierarchical memory, vector recall, episodic memory, etc.). Rejected for core: DR-016 already delegates this to `contributeContext` tool plugins, which is the maximally flexible shape — operators compose whatever layer they want without core caring. Shipping any particular strategy as a first-class feature would privilege one approach over the many that exist.

**Checklist for contributors considering a new core feature:**

- [ ] First question: is this feature about *reaching humans through channels* or about *how the agent thinks*? If the latter, stop and ask whether the right place is a harness-wrapping tool plugin, not core.
- [ ] Second question: does at least one existing agent harness already ship this feature? If yes, and the harness is wrappable via `install.sh`, the default answer is "wrap the harness." A new core feature needs to name a reason the harness's version cannot be used (typically: the feature must operate *outside* any single tool's process, e.g. cross-channel routing, cross-tool identity, proactive triggering).
- [ ] Third question: if shipped, does the feature force core to know something about an individual harness's shape (skills file format, subagent protocol, tool schema dialect)? If yes, the feature violates DR-014 and should instead live in a tool plugin that wraps that harness.
- [ ] If the feature survives all three questions, it's probably bridging infrastructure — write it up as its own DR, extend the Coverage claim in DR-014 accordingly, and make sure the new primitive is minimal enough that a second harness could reuse it without knowing anything about the first.
...
- **0.11 (2026-04-15)** — Added DR-017 (nanogent is a minimum bridge, not an agent harness). Retroactive ratification of the scope held since v0.3.x: nanogent owns channel ingress / identity / permissions / turn orchestration / proactive turn injection / plugin lifecycle / history log, and delegates skills / subagents / sandboxed execution / file editing / code search / browser automation / prompt engineering / context compression to whichever harness a `tools/<harness>` plugin wraps. `tools/claude` is the reference thin shell-out; future `tools/opencode`, `tools/codex`, `tools/cursor-agent`, etc. drop in the same way with zero core changes. The built-in provider tool loop is explicitly the lowest-common-denominator path, not the preferred one. Added a scope-level positioning paragraph at the top of the Scope section pointing at DR-017. No code changes — documents and protects the existing scope boundary and provides a feature-request filter for future contributions. Four rejected alternatives recorded (nanogent-native skills, nanogent-native subagents, built-in coding tools, rich tool planner on raw providers) so the next contributor who proposes them can see they've been considered.
- **0.10 (2026-04-15)** — Added a "Coverage claim" block to DR-014 enumerating the use cases the three-hook surface (`execute` / `start` / `contributeContext`) plus ctx primitives (`ToolCtx` / `ToolStartCtx` / `ToolContextCtx`) can express: reactive capabilities, proactive triggers, derived-context injection, derived-index maintenance, and task-assignment / work-queue tools. States the operational rule explicitly ("new use cases attempt existing seams first; new plugin type is last resort and must name a primitive no ctx + hook combination can provide") and grounds it in the two reversed plugin types (scheduler v0.7.0, memory v0.5.0) which both turned out to be hooks in disguise. Added a matching checklist item to DR-014. No code changes — documentation clarifies the emergent payoff of DR-010 + DR-014 + DR-016.
